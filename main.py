...
def show_menu():
    ...
    while running:
        ...
        title_surface = title_font.render("Balkan Kombat", True, Global.BLACK)
...
def select_countries():
    ...
    while running:
        ...
        instruction = font.render(f"Player {current_player} must choose a country!", True, Global.WHITE)
...
def start_game(player1_country, player2_country):
    width, height = Global.WIDTH, Global.HEIGHT
    screen = pygame.display.set_mode((width, height))
    """background = pygame.image.load("amaxresdefault.jpg").convert()
    ...
    while running:
        ...
        if (hp1 <= 0 or hp2 <= 0) and not winner_text:
            if hp1 <= 0 < hp2:
                rw2 += 1
                template = f"{player2_country} megnyerte a(z) {current_round}. kört!"
            elif hp2 <= 0 < hp1:
                rw1 += 1
                template = f"{player1_country} megnyerte a(z) {current_round}. kört!"
            else:
                rw1 += 0.5
                rw2 += 0.5
                template = f"Döntetlen a(z) {current_round}. körben!"
...
start_game(selected_countries[0], selected_countries[1])
